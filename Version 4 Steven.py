...
while True:
    ...
    if choice == 1:
        ...
        while True:
            ...
            if admin_choice == '1':
                print("\nReservation records:")
                records = reservation_system.display_records()
                if records:
                    for (row, col), (name, student_number) in records.items():
                        print(f"Seat ({row}, {col}) - Reserved by {name} (Student Number: {student_number})")
                else:
                ...
                while True:
                    try:
                        ...
                        col = int(input("Enter the column number of the reservation to cancel: "))
                        if not (1 <= row <= 100) or not (1 <= col <= 100):
                            print("Invalid input. Row and column numbers should be between 1 and 100.")
                            continue
                        break
                        ...
                        print("Invalid input. Please enter valid row and column numbers.")
                        continue
                reservation_system.cancel_reservation(row, col)
            elif admin_choice == '3':
                break
            else:
